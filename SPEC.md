...
#### 4.9.1 對話練習（M6，`#/interview/talk`）

模擬面試是「一題一題跳」，真面試不是——你答完會被**往下追問**。這個模式練的就是被追下去不斷線。

- 抽 **3 題**，配方 `self_intro ×1 → experience ×2`；只收 `follow_ups` 有 **3 層以上**的題目。
- 每題跑成一串輪次：**主問題 60 秒 → 追問 1 / 2 / 3 各 45 秒**。
  進入每一輪時自動播出該輪問題（一定發生在點擊事件內，才吃得到 iOS 的 TTS 解鎖，§5-1）。
  計時歸零不會擋住流程，只換一行字：「答得順就繼續，卡住也繼續——面試不會等你」。
- 每一輪可以「偷看回答方向」（`follow_ups[].tip_zh`），預設收合。
- 一串跑完才自評 **接得住／中間斷掉了**，然後攤開範答與每一層追問的回答方向。
  自評寫進 `store.interview[id]`，與模擬面試共用；**同一題今天第一次自評才算一次每日任務**。
- ⚠ **追問是題庫裡預錄的，不是即時生成**——App 沒有 LLM（附錄 B）。
  練的是「被問下去不斷線」，不是即興應答。真正的雙向對話仍然走 `SPEAKING.md` 的語音場景。
- `validate.js` 強制：`self_intro` 與 `experience` 的每題都要有 ≥3 個 `follow_ups`，且每個都要有 `tip_zh`。
  少一個就會出現「答完沒東西看」的空畫面，所以擋在驗證層。
...
| 週 | 面試格練什麼 | 跟讀 | 循環聽 | 模擬 | 週末前要生出來的東西 |
|---|---|---|---|---|---|
| 1 | **對話練習**：自我介紹 | 5 句 | 5 分 | 1 | 60 秒與 30 秒兩個版本的自我介紹逐字稿都定稿 |
| 2 | **對話練習**：自我經歷 | 6 句 | 5 分 | 1 | 3 個經歷故事寫成完整段落，每個都有一個數字 |
| 3 | **對話練習**：工作經歷與時間線 | 6 句 | 7 分 | 1 | 三年時間線一口氣講完不打結，換工作理由前後一致 |
| 4 | 動機與職涯 | 8 句 | 7 分 | 1 | 動機答案定稿，且與第 3 週的換工作理由對得起來 |
| 5 | 技術與行為題 | 8 句 | 7 分 | **2** | 三條技術主線各講 90 秒 ＋ 4 個 STAR 故事成句 |
| 6 | 薪資反問與收斂 | 6 句 | 5 分 | **2** | 薪資一句話版 ＋ 反問清單 5 題；模擬 6 題全部「答得出來」 |

- **前三週的面試格一律是 §4.9.1 的對話練習**（`WEEKS[].talkWeek`），只練自我介紹／自我經歷／工作經歷，
  每題被追問三層；第 4 週起才換回一般面試題並展開其他類別。
- **自我介紹每週都滾一次**：每週第一個面試日的提示固定從自我介紹開始（`WEEKS[].ivOpen`），
  它不是第 1 週做完就結束的作業。
- 第 5、6 週把該週第 3 格的面試題換成第二場模擬面試（`extraMock`），主題覆蓋不變。
- 代價要講清楚：技術題與行為題被壓到第 5 週才開始，只剩兩週。這是「先把自我介紹與經歷講到滾瓜爛熟」
  的代價，不是排程失誤。真面試若技術題吃重，要自己把 W5 的密度往前挪。
...
| 天 | 場景 | 重點 |
|---|---|---|
| D7、D14、D21 | S6 一般面試官 | 對應前三週：自我介紹 → 經歷深挖 → 工作經歷時間線 |
| D28、D35 | S8 壓力面試 | 離職原因與動機的刁鑽問法；行為題被質疑「那是你做的還是團隊做的」 |
| D32 | S7 技術追問 | 被連續追問技術細節不崩 |
| D38、D40 | S9 全真整場 | 45 分鐘完整流程，含反問與薪資；最後一場刻意排在面試前兩天 |
...
- [x] 前三週的面試格全部是對話練習，第 4 週起換回一般面試題　←　`test-daily.mjs` [24]
- [x] 對話練習沿用 `interview` kind，只換連結與文案　←　`test-daily.mjs` [24]
- [x] `#/interview/talk` 跑得完一整串（主問題＋三層追問→自評→範答）　←　`scripts/smoke.mjs` [13]
- [x] `self_intro` / `experience` 每題都有 ≥3 追問且每個都有 `tip_zh`　←　`scripts/validate.js`
